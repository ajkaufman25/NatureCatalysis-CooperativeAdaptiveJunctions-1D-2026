...
def figure_model_framework(outdir,model):
    ...
    save_figure(fig,outdir/'Figure_S15_model_framework')
...
def figure_band_diagrams(outdir,states):
    ...
    fig.savefig(outdir/'Figure_S16_conventional_band_diagrams.pdf',bbox_inches='tight')
    fig.savefig(outdir/'Figure_S16_conventional_band_diagrams.png',dpi=300,bbox_inches='tight');plt.close(fig)
...
def figure_controls(outdir,bb_jv,bb_op,abdf,ab_vmax,badf,ba_vmax,ba_sweep,ba_dg):
    ...
    save_figure(fig,outdir/'Figure_S17_adaptive_buried_controls')
...
def figure_native_sensitivity(outdir,gdf):
    ...
    save_figure(fig,outdir/'Figure_S18_native_asymmetry_sensitivity')
...
def figure_rate_intensity(outdir,kdf,idf):
    ...
    save_figure(fig,outdir/'Figure_S19_rate_and_intensity_sensitivity')
...
def run(outdir:Path,full_sweeps=False,precomputed_dir:Optional[Path]=None):
    ...
    if precomputed_dir is not None:
        for name in ('AB_reverse_bias_extended_Voc_sweep.csv','AB_catalyst_off_Voc_sweep.csv'):
            cand=precomputed_dir/name
            if cand.exists(): ab_pre=cand; break
        for name in ('BA_catalyst_off_open_circuit_sweep.csv','BA_catalyst_off_Voc_sweep.csv'):
            cand=precomputed_dir/name
            if cand.exists(): ba_pre=cand; break
    ...
    # Figures S15-S17 use only the final architecture logic.
    ...
    # Existing AA sensitivity figures S18-S19.
